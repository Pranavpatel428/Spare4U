...
public class EditItemMasterPage extends AppCompatActivity {
    ...
    String fetch_make_url = Constants.MAIN_URL + "api/make/list";
    String fetch_year_url = Constants.MAIN_URL + "api/year/list";
    String fetch_side_url = Constants.MAIN_URL + "api/side/list";
    String fetch_group_url = Constants.MAIN_URL + "api/group/list";
    String submit_data_url = Constants.MAIN_URL + "api/item/update";
    String authToken,selected_item_id,auth_date,auth_user,auth_date_time;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_edit_item_master_page);
        ...
        selected_item_id = preferences.getString("selected_item_id","");

        fetch_item_url = Constants.MAIN_URL + "api/item/" + selected_item_id;
    ...
    private void SubmitData() {
        StringRequest request = new StringRequest(Request.Method.POST, submit_data_url, response -> {
            ...
            protected Map<String, String> getParams() throws AuthFailureError {
                ...
                params.put("id",selected_item_id);
                ...
                params.put("auth_data",auth_date);
                params.put("auth_user",auth_user);
                params.put("suth_datetime",auth_date_time);
                return params;
            }
            ...
            try {
                ...
                if (jsonObject.getString("success").equals("true"))
                {
                    ...
                    for (int i = 0; i < jsonArray.length(); i++)
                    {
                        ...
                        itemMasterModel.setMake_id(v.getString("MAKE_ID"));
                        itemMasterModel.setMake_name(v.getJSONObject("makeMaster").getString("company_name"));
                        make_drop_down.setText(v.getJSONObject("makeMaster").getString("company_name"));
                        selected_make_id = v.getString("MAKE_ID");

                        itemMasterModel.setYear_id(v.getString("YEAR_ID"));
                        itemMasterModel.setYear_desc(v.getJSONObject("yearMaster").getString("YEAR_DESC"));
                        year_drop_down.setText(v.getJSONObject("yearMaster").getString("YEAR_DESC"));
                        selected_year_id = v.getString("YEAR_ID");

                        itemMasterModel.setSide_id(v.getString("SIDE_ID"));
                        itemMasterModel.setSide_desc(v.getJSONObject("sideMaster").getString("SIDE_DESC"));
                        side_drop_down.setText(v.getJSONObject("sideMaster").getString("SIDE_DESC"));
                        selected_side_id = v.getString("SIDE_ID");

                        itemMasterModel.setGroup_id(v.getString("GROUP_ID"));
                        itemMasterModel.setGroup_desc(v.getJSONObject("groupMaster").getString("GROUP_DESC"));
                        group_drop_down.setText(v.getJSONObject("groupMaster").getString("GROUP_DESC"));
                        selected_group_id = v.getString("GROUP_ID");

                        itemMasterModel.setItem_name(v.getString("ITEM_NAME"));
                        itemMasterModel.setItem_desc(v.getString("ITEM_DESC"));
                        itemMasterModel.setItem_price(v.getString("ITEM_PRICE"));
                        itemMasterModel.setItem_video_url(v.getString("ITEM_VIDEO_URL"));

                        Glide.with(this).load(v.getString("ITEM_PICT_URL1")).into(image_1_img);

                        Glide.with(this).load(v.getString("ITEM_PICT_URL2")).into(image_2_img);

                        Glide.with(this).load(v.getString("ITEM_PICT_URL3")).into(image_3_img);

                        Glide.with(this).load(v.getString("ITEM_PICT_URL4")).into(image_4_img);

                        Glide.with(this).load(v.getString("ITEM_PICT_URL5")).into(image_5_img);

                        auth_date = v.getString("AUTHORIZE_DATA");
                        auth_user = v.getString("AUTHORIZE_USER");
                        auth_date_time = v.getString("AUTHORIZE_DATETIME");
